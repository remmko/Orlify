...
class users {
    ...
    public function auth($username, $password) {
        $sql =
            "select id, username, password_hash, role from users where username =:user;";
        ...
        if (
            is_array($result) &&
            $result["password_hash"] == hash("sha256", $password)
        ) {
            return $result;
        } else {
            return false;
    ...
    public function getInfo($userID) {
        $sql =
            "select name, last_name, email, avatar from users where id =:userID;";
    ...
    public function register($username, $name, $surename, $email, $password, $route, $grups, $getGroups) {
        ...
        for ($i = 0; $i < count($grups); $i++) {
            if ($grups[$i] != null) {
                $sql =
                    "INSERT INTO user_grups (user_id, grup_id, aproved) VALUES (:userID, :grupID, 0);";
                $stmt = $this->sql->prepare($sql);
                $stmt->execute([
                    ":userID" => $getID["id"],
    ...
    public function acceptStudent($userID, $grupID) {
        $sql1 =
            "UPDATE user_grups SET aproved = 1 WHERE user_id = :userID AND grup_id = :grupID;";
    ...
    public function denyStudent($userID, $grupID) {
        $sql =
            "DELETE FROM user_grups WHERE user_id = :userID AND grup_id = :grupID;";
